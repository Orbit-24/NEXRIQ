...
async function seedUsers() {
  try {
    ...
    // Ensure full_admin users have all ADMIN_ONLY_TEAMS in their teams array
    const ADMIN_ONLY_TEAMS = ["2027/28 Test Page"];
    await User.updateMany(
      { permLevel: 'full_admin', teams: { $not: { $all: ADMIN_ONLY_TEAMS } } },
      { $addToSet: { teams: { $each: ADMIN_ONLY_TEAMS } } }
    );
